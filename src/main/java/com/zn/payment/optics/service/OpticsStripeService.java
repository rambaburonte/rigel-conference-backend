...
@Service
@Slf4j
public class OpticsStripeService {
    ...
    @Autowired
    private IOpticsPricingConfigRepository pricingConfigRepository;
    ...
    // Registration form integration is temporarily disabled until this is fixed
    @Autowired
    private IOpricsRegistrationFormRepository registrationFormRepository;
     ...
     */
    private boolean isDiscountPayment(OpticsPaymentRecord paymentRecord) {
    ...
    private void autoSyncDiscountOnPaymentUpdate(OpticsPaymentRecord paymentRecord) {
        ...
        try {
            ...
            String syncResult = paymentRecordRepository.syncOpticsBySessionId(paymentRecord.getSessionId());
            log.info("✅ Database sync result: {}", syncResult);
            
            // Fallback to manual sync if database function indicates no discount record exists
            if (syncResult != null && syncResult.contains("Only optics payment record exists")) {
                ...
                String secondSyncResult = paymentRecordRepository.syncOpticsBySessionId(paymentRecord.getSessionId());
     ...
     */
    public OpticsPaymentResponseDTO createCompleteResponseDTO(Session session, OpticsPaymentRecord paymentRecord) {
        OpticsPaymentResponseDTO responseDTO = new OpticsPaymentResponseDTO();
    ...
    try {
        ...
        OpticsPaymentRecord record = OpticsPaymentRecord.builder()
            ...
            .status(OpticsPaymentRecord.PaymentStatus.PENDING) // Initial status should be PENDING
     ...
     */
    public Session createCheckoutSessionWithPricing(CheckoutRequest request, OpticsPricingConfig pricingConfig) throws StripeException {
        ...
        try {
            ...
            OpticsPaymentRecord record = OpticsPaymentRecord.fromStripeWithPricing(
                session.getId(),
                request.getEmail(),
                "eur", // Always EUR
                pricingConfig,
     ...
     */
    public OpticsPaymentResponseDTO createValidatedCheckoutSession(CheckoutRequest request) throws StripeException {
        ...
        OpticsPricingConfig pricingConfig = pricingConfigRepository.findById(request.getPricingConfigId())
        ...
        OpticsPaymentRecord paymentRecord = paymentRecordRepository.findBySessionId(session.getId())
        ...
        OpticsPaymentResponseDTO response = createCompleteResponseDTO(session, paymentRecord);
     ...
     */
    public OpticsPaymentResponseDTO createCheckoutSessionWithPricingValidation(CheckoutRequest request, Long pricingConfigId) throws StripeException {
        ...
        OpticsPricingConfig pricingConfig = pricingConfigRepository.findById(pricingConfigId)
    ...
    private void processCheckoutSessionCompletedManual(String sessionId, String customerEmail, 
        ...
        try {
            ...
            OpticsPaymentRecord paymentRecord = paymentRecordRepository.findBySessionId(sessionId)
                .orElse(null);
            ...
            if (paymentRecord != null) {
                ...
                OpticsPaymentRecord newRecord = OpticsPaymentRecord.builder()
                        ...
                        .status(OpticsPaymentRecord.PaymentStatus.COMPLETED)
                ...
                OpticsPaymentRecord savedRecord = paymentRecordRepository.save(newRecord);
     ...
     */
    private void autoRegisterUserAfterPaymentManual(OpticsPaymentRecord paymentRecord, String customerEmail) {
    ...
    private void processCheckoutSessionCompleted(Session session) {
        ...
        try {
            ...
            if (paymentRecord != null) {
                ...
                OpticsPaymentRecord savedRecord = paymentRecordRepository.save(record);
                log.info("💾 ✅ Created new PaymentRecord ID: {} for session: {} with paymentStatus '{}'", 
                        savedRecord.getId(), session.getId(), savedRecord.getPaymentStatus());
    ...
    private void processPaymentIntentUpdate(com.stripe.model.PaymentIntent paymentIntent) {
        try {
            ...
            OpticsPaymentRecord existingRecord = paymentRecordRepository.findByPaymentIntentId(paymentIntent.getId())
                .orElse(null);
            ...
            if (existingRecord != null) {
                ...
                List<OpticsPaymentRecord> pendingRecords = paymentRecordRepository.findByStatus(OpticsPaymentRecord.PaymentStatus.PENDING);
                ...
                if (!pendingRecords.isEmpty()) {
                    ...
                    OpticsPaymentRecord recordToUpdate = findBestMatchingPendingRecord(pendingRecords, paymentIntent);
     ...
     */
    private void updateExistingPaymentRecord(OpticsPaymentRecord record, com.stripe.model.PaymentIntent paymentIntent) {
        log.info("🔄 Updating existing PaymentRecord ID: {} for payment intent: {}", record.getId(), paymentIntent.getId());
        
        record.setStatus(OpticsPaymentRecord.PaymentStatus.COMPLETED);
     ...
     */
    private OpticsPaymentRecord findBestMatchingPendingRecord(List<OpticsPaymentRecord> pendingRecords, com.stripe.model.PaymentIntent paymentIntent) {
        if (paymentIntent.getAmount() != null) {
            ...
            for (OpticsPaymentRecord record : pendingRecords) {
        ...
        OpticsPaymentRecord firstRecord = pendingRecords.get(0);
     ...
     */
    private void updatePendingPaymentRecord(OpticsPaymentRecord record, com.stripe.model.PaymentIntent paymentIntent) {
    ...
    private void createNewPaymentRecord(com.stripe.model.PaymentIntent paymentIntent) {
        ...
        if (existsInPaymentTable) {
            log.info("⚠️ Payment intent {} already exists in OpticsPaymentRecord table, skipping new payment record creation", paymentIntent.getId());
            return;
        }
    ...
    private void handlePaymentIntentFailed(Event event) {
        ...
        if (dataObjectDeserializer.getObject().isPresent()) {
            try {
                ...
                if (existingRecord != null) {
                    existingRecord.setStatus(OpticsPaymentRecord.PaymentStatus.FAILED);
    ...
    private void handleCheckoutSessionExpired(Event event) {
        ...
        if (dataObjectDeserializer.getObject().isPresent()) {
            try {
                ...
                OpticsPaymentRecord existingRecord = paymentRecordRepository.findBySessionId(session.getId())
                    .orElse(null);
                
                if (existingRecord != null) {
                    existingRecord.setStatus(OpticsPaymentRecord.PaymentStatus.EXPIRED);
    ...
    public OpticsPaymentResponseDTO retrieveSession(String sessionId) throws StripeException {
    ...
    public OpticsPaymentResponseDTO expireSession(String sessionId) throws StripeException {
     ...
     */
    private void autoRegisterUserAfterPayment(OpticsPaymentRecord paymentRecord, Session session) {
     ...
     */
    public OpticsPaymentRecord updatePaymentStatus(String sessionId) throws StripeException {
        log.info("🔄 Updating payment status for Optics session: {} (webhook-style processing)", sessionId);
        ...
        log.info("✅ Updated Optics payment record ID: {} for session: {} (webhook-style)", 
     ...
     */
    public OpticsPaymentRecord getPaymentStatus(String sessionId) {
        log.info("Getting payment status for Optics session: {}", sessionId);
        
        OpticsPaymentRecord paymentRecord = paymentRecordRepository.findBySessionId(sessionId)
                .orElseThrow(() -> new RuntimeException("Payment record not found for session: " + sessionId));
        
        log.info("✅ Retrieved Optics payment record for session: {}", sessionId);
     ...
     */
    public OpticsPaymentRecord getPaymentStatusFromProvider(String sessionId) throws StripeException {
        log.info("🔄 Getting real-time payment status from provider for Optics session: {}", sessionId);
     ...
     */
    private OpticsPaymentRecord getStripeSessionStatus(String sessionId) throws StripeException {
        ...
        try {
            ...
            if ("complete".equals(stripeSession.getStatus())) {
                ...
                paymentRecord.setStatus(OpticsPaymentRecord.PaymentStatus.EXPIRED);
            } else if ("open".equals(stripeSession.getStatus())) {
                paymentRecord.setStatus(OpticsPaymentRecord.PaymentStatus.PENDING);
            }
            ...
            log.info("✅ Retrieved and updated Optics payment status from Stripe for session: {} - Status: {}, Payment Status: {}", 
     ...
     */
    private OpticsPaymentRecord getPayPalOrderStatus(String orderId) {
    ...
    public com.zn.payment.dto.PayPalOrderResponse createPayPalOrder(com.zn.payment.dto.PayPalCreateOrderRequest request) {
        ...
        try {
            ...
            log.error("❌ Error creating PayPal order for Optics: {}", e.getMessage(), e);
