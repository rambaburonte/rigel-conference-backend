...
public class RenewaleStripeService {
    ...
    public void initPayPalClient() {
        try {
            ...
            if ("sandbox".equalsIgnoreCase(paypalMode)) {
                ...
                log.info("🏗️ PayPal client initialized in SANDBOX mode for Renewable");
            } else {
                environment = new PayPalEnvironment.Live(paypalClientId, paypalClientSecret);
                log.info("🏗️ PayPal client initialized in LIVE mode for Renewable");
            }
            
            this.paypalClient = new PayPalHttpClient(environment);
            log.info("✅ PayPal SDK client successfully initialized for Renewable");
            
        } catch (Exception e) {
            log.error("❌ Failed to initialize PayPal client for Renewable: {}", e.getMessage(), e);
     ...
     */
    public RenewablePaymentRecord updatePaymentStatus(String sessionId) throws StripeException {
        log.info("🔄 Updating payment status for Renewable session: {} (webhook-style processing)", sessionId);
        ...
        RenewablePaymentRecord updatedRecord = paymentRecordRepository.save(paymentRecord);
        
        // 5. Auto-sync discount table (same as webhook does)
        autoSyncDiscountOnPaymentUpdate(updatedRecord);
        
        log.info("✅ Updated Renewable payment record ID: {} for session: {} (webhook-style)", 
     ...
     */
    public RenewablePaymentRecord getPaymentStatus(String sessionId) {
        log.info("Getting payment status for Renewable session: {}", sessionId);
        
        RenewablePaymentRecord paymentRecord = paymentRecordRepository.findBySessionId(sessionId)
                .orElseThrow(() -> new RuntimeException("Payment record not found for session: " + sessionId));
        
        log.info("✅ Retrieved Renewable payment record for session: {}", sessionId);
     ...
     */
    public RenewablePaymentRecord getPaymentStatusFromProvider(String sessionId) throws StripeException {
        log.info("🔄 Getting real-time payment status from provider for Renewable session: {}", sessionId);
     ...
     */
    private RenewablePaymentRecord getStripeSessionStatus(String sessionId) throws StripeException {
        ...
        try {
            ...
            if ("complete".equals(stripeSession.getStatus())) {
                ...
                paymentRecord.setStatus(RenewablePaymentRecord.PaymentStatus.EXPIRED);
            } else if ("open".equals(stripeSession.getStatus())) {
                paymentRecord.setStatus(RenewablePaymentRecord.PaymentStatus.PENDING);
            }
            ...
            log.info("✅ Retrieved and updated Renewable payment status from Stripe for session: {} - Status: {}, Payment Status: {}", 
     ...
     */
    private RenewablePaymentRecord getPayPalOrderStatus(String orderId) {
        log.info("🔄 Fetching real-time PayPal order status for: {}", orderId);
        
        try {
            // Find existing payment record
            RenewablePaymentRecord paymentRecord = paymentRecordRepository.findBySessionId(orderId)
    ...
    /**
     * Create PayPal order for Renewable payments - PRODUCTION READY
     */
    public com.zn.payment.dto.PayPalOrderResponse createPayPalOrder(com.zn.payment.dto.PayPalCreateOrderRequest request) {
        log.info("Creating PayPal order for Renewable - Amount: {} {}, Customer: {}", 
        ...
        try {
            ...
            RenewablePricingConfig pricingConfig = null;
            if (request.getPricingConfigId() != null) {
            ...
            if (existingRegistration != null) {
                ...
                registrationForm = new RenewableRegistrationForm();
            ...
            RenewablePaymentRecord paymentRecord = RenewablePaymentRecord.builder()
                    ...
                    .provider("PAYPAL")
                    .status(RenewablePaymentRecord.PaymentStatus.PENDING)
            ...
            log.error("❌ Error creating PayPal order for Renewable: {}", e.getMessage(), e);
    ...
    private String createPayPalOrderAPI(com.zn.payment.dto.PayPalCreateOrderRequest request, String orderId) {
        try {
            ...
            log.info("✅ PayPal order created successfully for Renewable: {} with status: {}", order.id(), order.status());
    ...
    /**
     * Capture PayPal order for Renewable payments - PRODUCTION READY
     */
    public com.zn.payment.dto.PayPalOrderResponse capturePayPalOrder(String orderId) {
        log.info("Capturing PayPal order for Renewable: {} (called from return URL)", orderId);
        ...
        try {
            ...
            if (captureSuccess) {
                ...
                // Save updated record
                RenewablePaymentRecord updatedRecord = paymentRecordRepository.save(paymentRecord);
                log.info("✅ PayPal order captured for Renewable: {}", orderId);
                
                // Update registration form if linked
                if (updatedRecord.getRegistrationForm() != null) {
                    RenewableRegistrationForm registrationForm = updatedRecord.getRegistrationForm();
                ...
                paymentRecord.setStatus(RenewablePaymentRecord.PaymentStatus.FAILED);
            ...
            log.error("❌ Error capturing PayPal order for Renewable: {}", e.getMessage(), e);
    ...
    private boolean capturePayPalOrderAPI(String orderId, BigDecimal amount) {
        try {
            log.info("🔗 Capturing PayPal order via SDK for Renewable: {} for amount: {} EUR", orderId, amount);
            ...
            log.info("✅ PayPal order captured successfully for Renewable: {} with status: {}", 
            ...
            if ("COMPLETED".equals(capturedOrder.status())) {
                ...
                if (capturedOrder.purchaseUnits() != null && !capturedOrder.purchaseUnits().isEmpty()) {
                    ...
                    if (purchaseUnit.payments() != null && 
                        ...
                        if (amount.toString().equals(capturedAmount)) {
                            log.info("✅ PayPal capture amount verified for Renewable: {} EUR", capturedAmount);
                            return true;
                        } else {
                            log.error("❌ PayPal capture amount mismatch for Renewable. Expected: {} EUR, Captured: {} EUR", 
                ...
                log.warn("⚠️ PayPal capture completed but couldn't verify amount for Renewable order: {}", paypalOrderId);
                return true;
            } else {
                log.error("❌ PayPal capture failed with status: {} for Renewable order: {}", 
                        capturedOrder.status(), paypalOrderId);
                return false;
            }
            
        } catch (Exception e) {
            log.error("❌ Error capturing PayPal order via SDK for Renewable: {}", e.getMessage(), e);
            return false;
        }
    }
    
    /**
     * Extract PayPal order ID from internal order ID for Renewable
     */
    private String extractPayPalOrderId(String internalOrderId) {
        try {
            java.util.Optional<RenewablePaymentRecord> paymentRecordOpt = 
            ...
            if (paymentRecordOpt.isPresent()) {
                ...
                log.warn("⚠️ Using internal order ID as PayPal order ID for Renewable (should be updated in production): {}", 
                        internalOrderId);
            ...
            log.error("❌ Error extracting PayPal order ID for Renewable: {}", e.getMessage(), e);
            return null;
        }
    }
    ...
    public com.zn.payment.dto.PayPalOrderResponse handlePayPalReturn(String token, String payerId) {
        log.info("Handling PayPal return for Renewable - token: {} and payer: {}", token, payerId);
        ...
        try {
            ...
            log.error("❌ Error handling PayPal return for Renewable: {}", e.getMessage(), e);
