...
public class PolymersDiscountsService {
    ...
    private PayPalHttpClient getPayPalClient() {
        if (paypalClient == null) {
            ...
            log.info("✅ PayPal client initialized for Polymers in {} mode", paypalMode);
        }
        return paypalClient;
    }
    /**
     ...
     */
    public PolymersDiscounts getPaymentStatusFromProvider(String sessionId) throws StripeException {
        log.info("🔄 Getting real-time payment status from provider for Polymers discount session: {}", sessionId);
     ...
     */
    private PolymersDiscounts getStripeDiscountSessionStatus(String sessionId) throws StripeException {
        ...
        try {
            ...
            PolymersDiscounts discountRecord = discountsRepository.findBySessionId(sessionId);
            ...
            if ("complete".equals(stripeSession.getStatus())) {
                discountRecord.setStatus(com.zn.payment.polymers.entity.PolymersPaymentRecord.PaymentStatus.COMPLETED);
            } else if ("expired".equals(stripeSession.getStatus())) {
                discountRecord.setStatus(com.zn.payment.polymers.entity.PolymersPaymentRecord.PaymentStatus.EXPIRED);
            } else if ("open".equals(stripeSession.getStatus())) {
                discountRecord.setStatus(com.zn.payment.polymers.entity.PolymersPaymentRecord.PaymentStatus.PENDING);
            }
            ...
            // Save updated record
            PolymersDiscounts updatedRecord = discountsRepository.save(discountRecord);
            
            log.info("✅ Retrieved and updated Polymers discount status from Stripe for session: {} - Status: {}, Payment Status: {}", 
     ...
     */
    private PolymersDiscounts getPayPalDiscountOrderStatus(String orderId) {
        ...
        PolymersDiscounts discountRecord = discountsRepository.findBySessionId(actualSessionId);
    ...
    // ===== PAYPAL INTEGRATION FOR POLYMERS DISCOUNTS =====

    /**
     * Create PayPal order for Polymers discount payments - PRODUCTION LEVEL
    ...
    public PayPalOrderResponse createPayPalOrder(PayPalCreateOrderRequest request) {
        log.info("Creating PayPal discount order for Polymers - Amount: {} {}, Customer: {}", 
        ...
        try {
            ...
            PolymersDiscounts discountRecord = new PolymersDiscounts();
            ...
            PolymersDiscounts savedRecord = discountsRepository.save(discountRecord);
            log.info("💾 Saved Polymers PayPal discount record with ID: {} for PayPal order: {}", 
                    savedRecord.getId(), paypalOrderId);
            
            log.info("✅ Created PayPal discount order for Polymers: {} with approval URL: {}", paypalOrderId, approvalUrl);
    ...
    private PayPalOrderResult createPayPalOrderAPI(PayPalCreateOrderRequest request) {
        try {
            log.info("🚀 Creating real PayPal discount order via SDK for Polymers");
            ...
            ApplicationContext applicationContext = new ApplicationContext()
                ...
                .brandName("Polymers Science Conference - Discount")
                .landingPage("BILLING")
                .userAction("PAY_NOW");
            ...
            PurchaseUnitRequest purchaseUnitRequest = new PurchaseUnitRequest()
                .referenceId("POLYMERS_DISCOUNT_" + System.currentTimeMillis())
                .description("Polymers Science Conference Discount - " + request.getCustomerEmail())
                .customId("POLYMERS_DISCOUNT_" + System.currentTimeMillis())
            ...
            log.info("✅ PayPal discount order created successfully for Polymers: {} with status: {}", order.id(), order.status());
            ...
            log.info("🔗 PayPal discount approval URL generated for Polymers: {}", approvalUrl);
            
            return new PayPalOrderResult(order.id(), approvalUrl);
            
        } catch (Exception e) {
            log.error("❌ Error creating PayPal discount order via SDK for Polymers: {}", e.getMessage(), e);
            throw new RuntimeException("PayPal SDK error: " + e.getMessage(), e);
        }
    }

    /**
     * Capture PayPal order for Polymers discount payments
     */
    public PayPalOrderResponse capturePayPalOrder(String orderId) {
        ...
        try {
            ...
            log.info("✅ PayPal discount order captured successfully for Polymers: {} with status: {}", order.id(), order.status());
            
            // Update discount record in database
            PolymersDiscounts discountRecord = discountsRepository.findBySessionId(orderId);
            if (discountRecord != null) {
                ...
                PolymersDiscounts updatedRecord = discountsRepository.save(discountRecord);
                log.info("✅ Updated Polymers discount record status to COMPLETED for PayPal order: {}", orderId);
                ...
                log.error("❌ PolymersDiscounts record not found for PayPal order ID: {}", orderId);
                return PayPalOrderResponse.error("discount_record_not_found");
            }
            
        } catch (Exception e) {
            log.error("❌ Error capturing PayPal discount order for Polymers {}: {}", orderId, e.getMessage(), e);
